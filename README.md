# fBanner

> **The moment you copy, your banner image is ready.**

macOS Menu Bar App for instant banner image generation. Copy a URL or text, and fBanner automatically creates a banner image for you.

## Features

- **Instant Access** - `Cmd+Shift+K` shortcut for immediate access
- **Clipboard Automation** - Auto-detects copied URL/text
- **Versatile Formats** - PNG download, grid layout adjustment
- **Menu Bar Integration** - Lives quietly in your system menu bar
- **Generation History** - Reuse previously generated images
- **Multi-language Support** - English, Korean, Japanese, Chinese

## Requirements

- macOS 14.0 or later

## Product Page

| Language | Link |
|----------|------|
| English | [fBanner - Product Page](http://finfra.kr/product/fBanner/en/index.html) |
| Korean | [fBanner - 제품 페이지](http://finfra.kr/product/fBanner/kr/index.html) |

## Documentation

| Document | Description |
|----------|-------------|
| [Manual](./manual/) | User manual (KR/EN) |
| [REST API](./api/) | REST API reference & OpenAPI spec |
| [MCP Server](./mcp/) | Model Context Protocol server |
| [Claude Code Skill](./agents/claude/) | Claude Code plugin |
| [Localization](./localization/) | Multi-language string resources |
| [Resource](./resource/) | Example input/output files for testing |

## Community & Support

### Issues
- [GitHub Issues](https://github.com/Finfra/fBanner_public/issues)

### Board (English)
| Category | Link |
|----------|------|
| Notice | [fBanner Notice](https://finfra.kr/w1/category/fbanner-notice/) |
| Guide | [fBanner Guide](https://finfra.kr/w1/category/fbanner-guide/) |
| QnA | [fBanner QnA](https://finfra.kr/w1/category/fbanner-qna/) |
| Feedback | [fBanner Feedback](https://finfra.kr/w1/category/fbanner-feedback/) |

### Board (Korean)
| Category | Link |
|----------|------|
| Notice | [fBanner 공지](https://finfra.kr/w1/category/fbanner-notice-kr/) |
| Guide | [fBanner 사용법](https://finfra.kr/w1/category/fbanner-guide-kr/) |
| QnA | [fBanner QnA](https://finfra.kr/w1/category/fbanner-qna-kr/) |
| Feedback | [fBanner 피드백](https://finfra.kr/w1/category/fbanner-feedback-kr/) |

## License

Copyright (c) finfra.kr. All rights reserved.
