...
curl -X POST http://localhost:3011/api/split \
  -H "Content-Type: application/json" \
  -d '{"path":"resource/contents/example1.png","outputDir":"resource/contents_result","rows":3,"cols":4}'
...
| `--output=<dir>` | 출력 디렉토리 | `resource/contents_result` |
| `--ratio-w=<N>` | 가로 비율 | `1.0` |
| `--ratio-h=<N>` | 세로 비율 | `1.0` |
